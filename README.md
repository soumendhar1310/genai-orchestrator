...
- reserve OpenAI-driven generation for selected service/controller classes in later attempts when `OPENAI_API_KEY` is available
- keep attempt 1 deterministic and repository-focused without OpenAI generation
- retain existing validation guardrails and heuristic fallback when generated output is unsafe or invalid
- reject additional risky NUnit assertion shapes that have caused compile failures
